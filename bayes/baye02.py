...
def _trainNB0(trainMatrix, trainCategory):
    ...
    pAbusive = sum(trainCategory) / float(numTrainDocs)  #通过使用sum求解出矩阵中1的个数
    # 构造单词出现次数列表
...
"""
_trainNB0存在一个问题，要计算多个概率的乘积以获得文档属于某个类别的概率，即计算 p(w0|1) * p(w1|1) * p(w2|1)。
 如果其中一个概率值为 0，那么最后的乘积也为 0。为降低这种影响，可以将所有词的出现数初始化为 1，并将分母初始化为 2 
（取1 或 2 的目的主要是为了保证分子和分母不为0
"""
